	...
	if [ $findCnt -gt 0 ]; then
		...
		for targetFile in "${ES5_TARGET_FILES[@]}"; do
			...
			cp $targetFile ${targetFile}.backup
		        sed -i 's/\"\"https:\/\/map.pstatic.net\/nrb\/styles\/basic\/\{z\}\/\{x\}\/\{y\}\.png\?mt\=bg\.ol\.ts\.ar\.lko"\/.*maxZoom:20/\"https:\/\/map.pstatic.net\/nrb\/styles\/basic\/1728003511\/\{z\}\/\{x\}\/\{y\}\.png\?mt\=bg\.ol\.ts\.ar\.lko"\,\{minZoom:6,maxZoom:19,continuousWorld:\!0/g' $targetFile
